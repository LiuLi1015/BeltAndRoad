...
function displayKnowledgeGraph(data) {
    ...
    var nodes = allNodes.map(node => ({
        ...
        classes: node.data.label.toLowerCase() // 添加一个样式类别，根据节点的 label 属性来区分不同类型的节点
    }));

    var edges = allEdges.map(edge => ({
        data: {
            id: edge.data.id,
    ...
    var style = [
        { selector: 'node', css: { 'label': 'data(name)', 'text-valign': 'center', 'text-halign': 'center', 'text-wrap': 'wrap', 'height': '150px', 'width': '150px', 'font-size': '25px' } },
        { selector: 'edge', css: { 'label': 'data(label)', 'width': 2, 'curve-style': 'bezier', 'control-point-step-size': 1000, 'line-color': 'gray', 'font-size': '22px' } },
        // 添加节点悬停时显示完整名称的样式
    ...
    var cy = cytoscape({
        ...
        elements: elements,
        style: style,
        layout: { name: 'cose', idealEdgeLength: 420, nodeOverlap: 120, padding: -200 }
    });
    ...
    // 找到孤立的节点并删除
    var isolatedNodes = cy.nodes().filter(node => node.degree() === 0);
